# Contribute to ce-base-image
Want to hack on the ce-base-image? Awesome! This page contains information that will help you set up a development environment for working on the source code.
...
### Create a new branch
First, ensure that your local repository is up-to-date with the latest version of ce-base-image. More details on [GitHub help](https://help.github.com/articles/syncing-a-fork/).
...
This is a bit more advanced but required to ensure a proper Git history of ce-base-image. Git allows you to [rebase](https://git-scm.com/docs/git-rebase) commits. In other words: it allows you to rewrite the commit history.
...
```sh
pick 911c35b Add "How to contribute to ce-base-image" tutorial
pick 33c8973 Begin workflow
pick 3502f2e Refactoring and typo fixes
```

In the case above we should merge the last 2 commits in the commit of this tutorial (`Add "How to contribute to ce-base-image" tutorial`). You can "squash" commits, i.e. merge two or more commits into a single one.
...
```sh
reword 911c35b Add "How to contribute to ce-base-image" tutorial
squash 33c8973 Begin workflow
squash 3502f2e Refactoring and typo fixes
```
...
ce-base-image enforces the DCO using the a [bot](https://github.com/probot/dco). You can view the details on the DCO check by viewing the `Checks` tab in the GitHub pull request.
...
ce-base-image uses [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/) as it's commit message format. These are particularly important as semantic releases are in use, and they use the commit messages to determine the type of changes in the codebase. Following formalized conventions for commit messages the semantic release automatically determines the next [semantic version](https://semver.org) number and generates a changelog based on the conventional commit.

Semantic releases originate in the [Angular Commit Message Conventions](https://github.com/angular/angular.js/blob/master/DEVELOPERS.md#-git-commit-guidelines), and the rules described there are the ones used by ce-base-image
...
### Open a pull request
We made a lot of progress. Good work. In this step we finally open a pull request to submit our additions. Open the [ce-base-image master repository](https://github.com/continuousengineeringproject/ce-base-image/) on GitHub in your browser.
...
Then the maintainers will review your PR, potentially initiate discussion around your change and finally, merge it successfully in ce-base-image. Congratulations !
